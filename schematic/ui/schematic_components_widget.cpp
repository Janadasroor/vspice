...
// ─── SymbolPreviewPopup ─────────────────────────────────────────────────────
class SymbolPreviewPopup : public QWidget {
public:
    explicit SymbolPreviewPopup(QWidget* parent = nullptr) : QWidget(parent, Qt::ToolTip | Qt::FramelessWindowHint) {
    ...
    void setSymbol(const SymbolDefinition& sym) {
        m_symbol = sym;
        update();
    }
...
protected:
    void paintEvent(QPaintEvent* event) override {
        ...
        QColor bg = (theme && theme->type() == PCBTheme::Light) ? QColor(255, 255, 255, 245) : QColor(30, 30, 35, 245);
        ...
        if (m_symbol.name().isEmpty()) return;

        // Filter primitives for Unit 1 and BodyStyle 1 (default)
        ...
        // Calculate bounding box of filtered primitives for scaling
        ...
        if (symbolRect.isEmpty()) return;

        // Reserve space for margins
        QRectF drawArea = rect().adjusted(35, 45, -35, -35);
        ...
        painter.scale(scale, -scale); // Flip Y to match schematic coordinates if needed, but wait: symbols are usually +Y down in Qt system? 
        // Schematic coordinate system in Viospice usually has Y increasing downwards, same as standard UI. 
        // Let's stick with standard for now unless it's upside down.
        painter.scale(1, -1); // Viospice symbols usually have Y up in internal definitions (Spice convention)? 
        // Looking at SymbolPrimitive::rotateCW: "center.y() + dx". 
        // Actually, let's NO scale -1 for now.
        painter.scale(1, 1); 
        
        painter.translate(-symbolRect.center());

        painter.setPen(QPen(fg, 1.5 / scale, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
        for (const auto& prim : filtered) {
...
private:
    void drawPrimitive(QPainter* p, const SymbolPrimitive& prim, const QColor& fg, qreal scale) {
        switch (prim.type) {
            case SymbolPrimitive::Line: {
            ...
            case SymbolPrimitive::Arc: {
                ...
                qreal h = prim.data.contains("height") ? prim.data["height"].toDouble() : prim.data["h"].toDouble();
                QRectF r(prim.data["x"].toDouble(), prim.data["y"].toDouble(), w, h);
                int start = prim.data["startAngle"].toInt();
                int span = prim.data["spanAngle"].toInt();
                p->drawArc(r, start, span); // Arc angles are already in 1/16th deg from SymbolPrimitive::createArc
                break;
            }
            case SymbolPrimitive::Polygon: {
                QPolygonF poly;
            ...
            case SymbolPrimitive::Pin: {
                QPointF pos(prim.data["x"].toDouble(), prim.data["y"].toDouble());
                qreal len = prim.data["length"].toDouble(15.0);
                QString orient = prim.data["orientation"].toString("Right");
                QPointF end = pos;
                if      (orient == "Right") end.setX(pos.x() + len);
                else if (orient == "Left")  end.setX(pos.x() - len);
                else if (orient == "Up")    end.setY(pos.y() - len);
                else if (orient == "Down")  end.setY(pos.y() + len);
                p->drawLine(pos, end);
                p->setBrush(fg);
                p->drawEllipse(pos, 1.5/scale, 1.5/scale);
                p->setBrush(Qt::NoBrush);
                break;
            }
            case SymbolPrimitive::Text: {
                QPointF pos(prim.data["x"].toDouble(), prim.data["y"].toDouble());
                int size = prim.data["fontSize"].toInt(8);
                p->save();
                p->setFont(QFont("Inter", size));
                p->drawText(pos, prim.data["text"].toString());
                p->restore();
                break;
            }
            case SymbolPrimitive::Bezier: {
                ...
                path.cubicTo(prim.data["x2"].toDouble(), prim.data["y2"].toDouble(),
                             prim.data["x3"].toDouble(), prim.data["y3"].toDouble(),
                             prim.data["x4"].toDouble(), prim.data["y4"].toDouble());
                p->drawPath(path);
                break;
            }
            default: break;
    ...
    : QWidget(parent)
{
    ...
    m_previewPopup = new SymbolPreviewPopup(this);
